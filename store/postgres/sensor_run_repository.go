...
func (repo *SensorRunRepository) Save(ctx context.Context, event models.JobEvent, namespaceSpec models.NamespaceSpec, jobSpec models.JobSpec) error {
...
func (repo *SensorRunRepository) Update(ctx context.Context, event models.JobEvent, namespaceSpec models.NamespaceSpec, jobSpec models.JobSpec) error {
	eventPayload := event.Value
	eventPayloadString, _ := json.Marshal(eventPayload)
	repo.logger.Info(string(eventPayloadString))
	...
	if err := repo.db.WithContext(ctx).Where("job_id = ? and project_id = ? and namespace_id = ? and scheduled_at = ? and atempt = ? ", jobSpec.ID, uuid.UUID(namespaceSpec.ProjectSpec.ID), namespaceSpec.ID, scheduledAtTimeStamp, attemptNumber).Find(&taskRun).Error; err != nil {
...
func NewSensorRunRepository(db *gorm.DB, logger log.Logger) *TaskRunRepository {
