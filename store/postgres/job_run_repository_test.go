...
func TestIntegrationJobRunMetricsRepository(t *testing.T) {
	...
	t.Run("Update", func(t *testing.T) {
		t.Run("should update job runs correctly", func(t *testing.T) {
			...
			assert.Equal(t, jobRunSpec.JobID, jobConfigs[0].ID)
			assert.Equal(t, jobRunSpec.NamespaceID, namespaceSpec.ID)
			assert.Equal(t, jobRunSpec.Attempt, jobEvent.Value["attempt"].GetNumberValue())
			assert.Equal(t, jobRunSpec.Status, "FINISHED")
			assert.Equal(t, jobRunSpec.Duration, int64(jobEvent.Value["attempt"].GetNumberValue()))
			assert.Equal(t, jobRunSpec.EndTime, time.Unix(int64(jobEvent.Value["event_time"].GetNumberValue()), 0))
		})
	})
