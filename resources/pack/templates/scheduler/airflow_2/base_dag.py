...
transformation_{{.Job.Task.Unit.Name | replace "-" "__dash__" | replace "." "__dot__"}} = SuperKubernetesPodOperator(
    ...
    env_vars = [
      ...
      k8s.V1EnvVar(name="TASK_TYPE",value='{{$.InstanceTypeTransformation}}'), 
      k8s.V1EnvVar(name="TASK_NAME",value='{{.Job.Task.Unit.Name}}'),
...
hook_{{$t.Unit.Name}} = SuperKubernetesPodOperator(
    ...
    env_vars = [
      ...
      k8s.V1EnvVar(name="TASK_NAME",value='{{$t.Unit.Name}}'), 
      k8s.V1EnvVar(name="SCHEDULED_AT",value='{{ "{{ next_execution_date }}" }}')
